...
class QuestionCard extends Component {
  render() {
    return (
      <Card
        className="quiz-card"
        title={"Question " + (this.props.questionNo + 1) + "/10"}
      >
        <div className="question-body">
                ...
                <div>
                  ...
                  <div>
                    ...
                    <span className="correct-answer">
                      {he.decode(this.props.correctOrIncorrect.correct_answer)}
                    </span>
                  </div>
                </div>
              )
          ...
          <div>
            <div className="question-seconds">
              {Math.ceil(this.props.timeLeft / 1000)} seconds left
            </div>
            <Progress
              className="question-progress"
              percent={100 - (this.props.timeLeft / 10000) * 100}
              showInfo={false}
              status="active"
              strokeWidth={24}
            />
            ...
            {this.props.alternatives.slice(0, 2).map((ans, i) => {
              return (
                <Col span={12} key={i}>
                  <Button
                    value={ans}
                    onClick={e => this.props.answerClick(e.target.value)}
                    type="default"
                    block
                  >
                    {he.decode(ans)}
